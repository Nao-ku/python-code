#importar libreria
import math 
#Ejemplo I
Ejemplo1 = 5*(2+5)
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo1}")

#Ejemplo II
Ejemplo2 = (4+3)-((3*2)+1)
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo2}")

#Ejemplo III
Ejemplo3 = 2*(3+(5*(9/3)))
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo3}")

#Ejemplo IIII
Ejemplo4 = 7+3*(1+5*(2*(9-7)+4)+2)
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo4}")

#Ejemplo IIIII
Ejemplo5 = 15/3*(7+(68-15*33+(math.pow(45,2)/5)/3)/2)+19
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo5}")

#Ejemplo IIIIII
Ejemplo6 = 15/3*(7+(68-15*33+(math.pow(45,2)/5)/3)/2)+19
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo6}")

#Ejemplo IIIIIII
Ejemplo7 =(16/4*((((4+8*2)/4- (3**2)+math.sqrt(4))/2)*math.sqrt(25))-3)+16
#Imprimir variable 
print(f"El resultado de la operacion es:{Ejemplo7}")

